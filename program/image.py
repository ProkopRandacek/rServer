...
class imageDB:
    ...
    def clear(self):
        ...
        for i in self.images:
            print(
                i.name,
                i.creationTime,
                nowTime - i.creationTime,
                (nowTime - i.creationTime) > c.image.maxAge,
            )
        ...
        log(
            [
                "clearing done",
                f"{n} images deleted in {int(1000*(time.time() - nowTime))/1000} seconds",
            ]
        )
    ...
    def uploadImage(self):
        # TODO
        if len(self.images) > int(62 * 62 * 62 / 100):
            self.clear()  # 2383 images -> if 1 img = 5mb -> 12gb
